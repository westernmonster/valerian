...
func (w *watcher) Handle(event Event) {
	...
	case w.ch <- event:
	default:
		log.Errorf("paladin: discard event:%+v", event)
	}
}
...
func (s *sven) load() (ver int64, err error) {
	...
	if v, err = s.check(-1); err != nil {
		log.Errorf("paladin: s.check(-1) error(%v)", err)
		return
	}
	for i := 0; i < _maxLoadRetries; i++ {
		...
		log.Errorf("paladin: s.config(%v) error(%v)", ver, err)
...
func (s *sven) watchproc(ver int64) {
	...
	for {
		...
		if err != nil {
			...
			log.Errorf("paladin: s.check(%d) error(%v)", ver, err)
			retry++
