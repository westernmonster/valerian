...
import (
	...
	"valerian/library/container/queue/aqm"
...
func New(c *aqm.Config) *Limiter {
	...
	go func() {
		...
		for {
			...
			v := l.rate.Stat()
			q := l.queue.Stat()
			log.Info(fmt.Sprintf("rate/limit: limit(%d) inFlight(%d) minRtt(%v) rtt(%v) codel packets(%d)", v.Limit, v.InFlight, v.MinRTT, v.LastRTT, q.Packets))
		}
	}()
	return l
}
