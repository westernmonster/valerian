...
func (d *Databus) dialInstance() (redis.Conn, error) {
	...
	// 	log.Warn(fmt.Sprintf("Databus: no instances(%s,%s) found in discovery,Use config(%s,%s)", _appid, env.Zone, d.conf.Proto, d.conf.Addr))
	// 	return redis.Dial(d.conf.Proto, d.conf.Addr, d.redisOptions()...)
	// }
...
func (d *Databus) disc() {
	if d.p != nil {
		...
		op.Close()
		op = nil
		log.Info(fmt.Sprintf("discovery event renew redis pool group(%s) topic(%s)", d.conf.Group, d.conf.Topic))
	}
	// if insMap, ok := d.dis.Fetch(context.Background()); ok {
	// 	if ins, ok := insMap[env.Zone]; ok && len(ins) > 0 {
	// 		log.Info(fmt.Sprintf("get databus instances len(%d)", len(ins)))
	// 	}
	// }
}
	...
	for {
		...
		if c == nil || c.Err() != nil {
			if c, err = d.d(); err != nil {
				log.Error(fmt.Sprintf("redis.Dial(%s@%s) group(%s) retry error(%v)", d.conf.Proto, d.conf.Addr, d.conf.Group, err))
				continue
			}
		}
		d.lock.RLock()
		...
		for k, v := range commit {
			if _, err = c.Do("SET", k, v); err != nil {
				c.Close()
				log.Error(fmt.Sprintf("group(%s) conn.Do(SET,%d,%d) commit error(%v)", d.conf.Group, k, v, err))
				break
			}
			delete(commit, k)
			commited[k] = v
		...
		if res, err = redis.ByteSlices(c.Do(_cmdSub, "")); err != nil {
			c.Close()
			log.Error(fmt.Sprintf("group(%s) conn.Do(MGET) error(%v)", d.conf.Group, err))
			continue
		}
		for _, r = range res {
			msg := &Message{d: d}
			if err = json.Unmarshal(r, msg); err != nil {
				log.Error(fmt.Sprintf("json.Unmarshal(%s) error(%v)", r, err))
				continue
			}
			d.msgs <- msg
		}
...
func (d *Databus) Send(c context.Context, k string, v interface{}) (err error) {
	...
	if b, err = json.Marshal(v); err != nil {
		log.Error(fmt.Sprintf("json.Marshal(%v) error(%v)", v, err))
		return
	}
	conn := d.p.Get(context.TODO())
	if _, err = conn.Do(_cmdPub, k, b); err != nil {
		log.Error(fmt.Sprintf("conn.Do(%s,%s,%s) error(%v)", _cmdPub, k, b, err))
	}
	conn.Close()
	return
}
