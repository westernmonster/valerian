...
func parseDSN(rawdsn string) *ServerConfig {
	...
	if err != nil {
		panic(errors.Wrapf(err, "mars: invalid dsn: %s", rawdsn))
	}
...
func (engine *Engine) Start() error {
	...
	if err != nil {
		errors.Wrapf(err, "mars: listen tcp: %s", conf.Address)
		return err
	}

	log.Info(fmt.Sprintf("mars: start http listen addr: %s", conf.Address))
	server := &http.Server{
	...
	go func() {
		if err := engine.RunServer(server, l); err != nil {
			if errors.Cause(err) == http.ErrServerClosed {
				log.Info("mars: server closed")
				return
			}
			panic(errors.Wrapf(err, "mars: engine.ListenServer(%+v, %+v)", server, l))
		}
	}()
...
func NewServer(conf *ServerConfig) *Engine {
	if conf == nil {
		if !flag.Parsed() {
			fmt.Fprint(os.Stderr, "[mars] please call flag.Parse() before Init warden server, some configure may not effect.\n")
		}
		conf = parseDSN(_httpDSN)
	} else {
		fmt.Fprintf(os.Stderr, "[mars] config will be deprecated, argument will be ignored. please use -http flag or HTTP env to configure http server.\n")
	}
...
func DefaultServer(conf *ServerConfig) *Engine {
	...
	engine.Use(Recovery(), Trace(tracing.GlobalTracer()), Logger(), CORS(), CSRF(), Mobile())
	return engine
}
...
func (engine *Engine) addRoute(method, path string, handlers ...HandlerFunc) {
	if path[0] != '/' {
		panic("mars: path must begin with '/'")
	}
	if method == "" {
		panic("mars: HTTP method can not be empty")
	}
	if len(handlers) == 0 {
		panic("mars: there must be at least one handler")
	}
...
func (engine *Engine) SetConfig(conf *ServerConfig) (err error) {
	if conf.Timeout <= 0 {
		return errors.New("mars: config timeout must greater than 0")
	}
	if conf.Network == "" {
		conf.Network = "tcp"
	}
