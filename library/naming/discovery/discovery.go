...
func fixConfig(c *Config) {
	if len(c.Nodes) == 0 {
		c.Nodes = []string{"discovery.stonote.lan"}
	}
	if env.Region != "" {
		c.Region = env.Region
	}
...
func New(c *Config) (d *Discovery) {
	if c == nil {
		c = &Config{
			Nodes:  []string{"discovery.stonote.lan"},
			Key:    "discovery",
			Secret: "discovery",
		}
	}
	...
	d.httpClient = mars.NewClient(cfg)
...
func (d *Discovery) Build(appid string) naming.Resolver {
	...
	log.Info(fmt.Sprintf("disocvery: AddWatch(%s) already watch(%v)", appid, ok))
	d.once.Do(func() {
		go d.serverproc()
	})
	return r
...
func (d *Discovery) cancel(ins *naming.Instance) (err error) {
	...
	if err = d.httpClient.Post(context.Background(), uri, "", params, &res); err != nil {
		d.switchNode()
		log.Error(fmt.Sprintf("discovery cancel client.Get(%v) env(%s) appid(%s) hostname(%s) error(%v)",
			uri, conf.Env, ins.AppID, conf.Host, err))
		return
	}
	if ec := ecode.Int(res.Code); !ec.Equal(ecode.OK) {
		log.Warn(fmt.Sprintf("discovery cancel client.Get(%v)  env(%s) appid(%s) hostname(%s) code(%v)",
			uri, conf.Env, ins.AppID, conf.Host, res.Code))
		err = ec
		return
	}
	log.Info(fmt.Sprintf("discovery cancel client.Get(%v)  env(%s) appid(%s) hostname(%s) success",
...
func (d *Discovery) register(ctx context.Context, ins *naming.Instance) (err error) {
	...
	if ins.Metadata != nil {
		if metadata, err = json.Marshal(ins.Metadata); err != nil {
			log.Error(fmt.Sprintf("discovery:register instance Marshal metadata(%v) failed!error(%v)", ins.Metadata, err))
		}
	}
	res := new(struct {
		Code    int    `json:"code"`
	...
	if ec := ecode.Int(res.Code); !ec.Equal(ecode.OK) {
		log.Warn(fmt.Sprintf("discovery: register client.Get(%v)  env(%s) appid(%s) addrs(%v)  code(%v)",
			uri, conf.Env, ins.AppID, ins.Addrs, res.Code))
		err = ec
		return
	}
	log.Info(fmt.Sprintf("discovery: register client.Get(%v) env(%s) appid(%s) addrs(%s) success",
...
func (d *Discovery) set(ctx context.Context, ins *naming.Instance) (err error) {
	...
	if ins.Metadata != nil {
		var metadata []byte
		if metadata, err = json.Marshal(ins.Metadata); err != nil {
			log.Error(fmt.Sprintf("discovery:set instance Marshal metadata(%v) failed!error(%v)", ins.Metadata, err))
		}
		params.Set("metadata", string(metadata))
	}
	if err = d.httpClient.Post(ctx, uri, "", params, &res); err != nil {
		d.switchNode()
		log.Error(fmt.Sprintf("discovery: set client.Get(%v)  zone(%s) env(%s) appid(%s) addrs(%v) error(%v)",
			uri, conf.Zone, conf.Env, ins.AppID, ins.Addrs, err))
		return
	}
	if ec := ecode.Int(res.Code); !ec.Equal(ecode.OK) {
		log.Warn(fmt.Sprintf("discovery: set client.Get(%v)  env(%s) appid(%s) addrs(%v)  code(%v)",
			uri, conf.Env, ins.AppID, ins.Addrs, res.Code))
		err = ec
		return
	}
	log.Info(fmt.Sprintf("discovery: set client.Get(%v) env(%s) appid(%s) addrs(%s) success",
...
func (d *Discovery) renew(ctx context.Context, ins *naming.Instance) (err error) {
	...
	if err = d.httpClient.Post(ctx, uri, "", params, &res); err != nil {
		d.switchNode()
		log.Error(fmt.Sprintf("discovery: renew client.Get(%v)  env(%s) appid(%s) hostname(%s) error(%v)",
			uri, conf.Env, ins.AppID, conf.Host, err))
		return
	}
	if ec := ecode.Int(res.Code); !ec.Equal(ecode.OK) {
		...
		log.Error(fmt.Sprintf("discovery: renew client.Get(%v) env(%s) appid(%s) hostname(%s) code(%v)",
			uri, conf.Env, ins.AppID, conf.Host, res.Code))
		return
	}
	return
...
func (d *Discovery) nodes() (nodes []string) {
	...
	if err := d.httpClient.Get(d.ctx, uri, "", nil, res); err != nil {
		d.switchNode()
		log.Error(fmt.Sprintf("discovery: consumer client.Get(%v)error(%+v)", uri, err))
		return
	}
	if ec := ecode.Int(res.Code); !ec.Equal(ecode.OK) {
		log.Error(fmt.Sprintf("discovery: consumer client.Get(%v) error(%v)", uri, res.Code))
		return
	}
	if len(res.Data) == 0 {
		log.Warn(fmt.Sprintf("discovery: get nodes(%s) failed,no nodes found!", uri))
		return
	}
...
func (d *Discovery) polls(ctx context.Context, host string) (apps map[string]appData, err error) {
	...
	if err = d.httpClient.Get(ctx, uri, "", params, res); err != nil {
		log.Error(fmt.Sprintf("discovery: client.Get(%s) error(%+v)", uri+"?"+params.Encode(), err))
		return
	}
	if ec := ecode.Int(res.Code); !ec.Equal(ecode.OK) {
		if !ec.Equal(ecode.NotModified) {
			log.Error(fmt.Sprintf("discovery: client.Get(%s) get error code(%d)", uri+"?"+params.Encode(), res.Code))
			err = ec
		}
		return
	}
	...
	for _, app := range res.Data {
		if app.LastTs == 0 {
			err = ecode.ServerErr
			log.Error(fmt.Sprintf("discovery: client.Get(%s) latest_timestamp is 0,instances:(%s)", uri+"?"+params.Encode(), info))
			return
		}
	}
	log.Info(fmt.Sprintf("discovery: polls uri(%s)", uri+"?"+params.Encode()))
	log.Info(fmt.Sprintf("discovery: successfully polls(%s) instances (%s)", uri+"?"+params.Encode(), info))
	apps = res.Data
	return
}
