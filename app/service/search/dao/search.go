...
func (p *Dao) AccountSearch(c context.Context, arg *model.BasicSearchParams, ids []int64) (res *model.SearchResult, err error) {
	...
	if arg.KW != "" {
		query = query.Must(elastic.NewMultiMatchQuery(arg.KW, arg.KwFields...).Type("best_fields").TieBreaker(0.1).Fuzziness("1").PrefixLength(2))
	}
...
func (p *Dao) DiscussionSearch(c context.Context, arg *model.BasicSearchParams, ids []int64) (res *model.SearchResult, err error) {
	...
	if arg.KW != "" {
		query = query.Must(elastic.NewMultiMatchQuery(arg.KW, arg.KwFields...).TieBreaker(0.1).Fuzziness("1").PrefixLength(2))
	}
