...
func (p *Dao) GetAccountTopicSettings(c context.Context, node sqalx.Node) (items []*model.AccountTopicSetting, err error) {
	...
	sqlSelect := "SELECT a.* FROM account_topic_settings a WHERE a.deleted=0 ORDER BY a.id DESC "
...
func (p *Dao) GetAccountTopicSettingsByCond(c context.Context, node sqalx.Node, cond map[string]interface{}) (items []*model.AccountTopicSetting, err error) {
	...
	sqlSelect := fmt.Sprintf("SELECT a.* FROM account_topic_settings a WHERE a.deleted=0 %s ORDER BY a.id DESC", clause)
...
func (p *Dao) GetAccountTopicSettingByID(c context.Context, node sqalx.Node, id int64) (item *model.AccountTopicSetting, err error) {
	...
	sqlSelect := "SELECT a.* FROM account_topic_settings a WHERE a.id=? AND a.deleted=0"
...
func (p *Dao) GetAccountTopicSettingByCond(c context.Context, node sqalx.Node, cond map[string]interface{}) (item *model.AccountTopicSetting, err error) {
	...
	sqlSelect := fmt.Sprintf("SELECT a.* FROM account_topic_settings a WHERE a.deleted=0 %s", clause)
...
func (p *Dao) AddAccountTopicSetting(c context.Context, node sqalx.Node, item *model.AccountTopicSetting) (err error) {
	sqlInsert := "INSERT INTO account_topic_settings( id,account_id,topic_id,important,mute_notification,fav,deleted,created_at,updated_at) VALUES ( ?,?,?,?,?,?,?,?,?)"

	if _, err = node.ExecContext(c, sqlInsert, item.ID, item.AccountID, item.TopicID, item.Important, item.MuteNotification, item.Fav, item.Deleted, item.CreatedAt, item.UpdatedAt); err != nil {
...
func (p *Dao) UpdateAccountTopicSetting(c context.Context, node sqalx.Node, item *model.AccountTopicSetting) (err error) {
	sqlUpdate := "UPDATE account_topic_settings SET account_id=?,topic_id=?,important=?,mute_notification=?,fav=?,updated_at=? WHERE id=?"

	_, err = node.ExecContext(c, sqlUpdate, item.AccountID, item.TopicID, item.Important, item.MuteNotification, item.Fav, item.UpdatedAt, item.ID)
	if err != nil {
