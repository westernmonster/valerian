...
import (
	...
	server "valerian/app/service/topic-feed/server/grpc"
	"valerian/app/service/topic-feed/server/http"
...
func main() {
	...
	for {
		s := <-c
		log.Infof("topic-feed-service get a signal %s", s.String())
		switch s {
		case syscall.SIGQUIT, syscall.SIGTERM, syscall.SIGINT:
			...
			srv.Close()
			log.Info("topic-feed-service exit")
			return
		case syscall.SIGHUP:
		default:
			return
