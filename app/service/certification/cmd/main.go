...
import (
	...
	server "valerian/app/service/certification/server/grpc"
	"valerian/app/service/certification/server/http"
...
func main() {
	...
	for {
		s := <-c
		log.Infof("certification-service get a signal %s", s.String())
		switch s {
		case syscall.SIGQUIT, syscall.SIGTERM, syscall.SIGINT:
			...
			srv.Close()
			log.Info("certification-service exit")
			return
		case syscall.SIGHUP:
		default:
			return
