...
import (
	...
	server "valerian/app/service/article/server/grpc"
	"valerian/app/service/article/server/http"
...
func main() {
	...
	for {
		s := <-c
		log.Infof("article-service get a signal %s", s.String())
		switch s {
