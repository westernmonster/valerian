...
func (p *Dao) SetArticleFileCache(c context.Context, articleID int64, m []*model.ArticleFileResp) (err error) {
...
func (p *Dao) ArticleFileCache(c context.Context, articleID int64) (m []*model.ArticleFileResp, err error) {
	key := articleFileKey(articleID)
	conn := p.mc.Get(c)
	defer conn.Close()
