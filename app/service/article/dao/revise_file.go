...
func (p *Dao) GetReviseFiles(c context.Context, node sqalx.Node) (items []*model.ReviseFile, err error) {
	...
	sqlSelect := "SELECT a.id,a.file_name,a.file_url,a.seq,a.revise_id,a.deleted,a.created_at,a.updated_at FROM revise_files a WHERE a.deleted=0 ORDER BY a.id DESC "
...
func (p *Dao) GetReviseFilesByCond(c context.Context, node sqalx.Node, cond map[string]interface{}) (items []*model.ReviseFile, err error) {
	...
	sqlSelect := fmt.Sprintf("SELECT a.id,a.file_name,a.file_url,a.seq,a.revise_id,a.deleted,a.created_at,a.updated_at FROM revise_files a WHERE a.deleted=0 %s ORDER BY a.id DESC", clause)
...
func (p *Dao) GetReviseFileByID(c context.Context, node sqalx.Node, id int64) (item *model.ReviseFile, err error) {
	...
	sqlSelect := "SELECT a.id,a.file_name,a.file_url,a.seq,a.revise_id,a.deleted,a.created_at,a.updated_at FROM revise_files a WHERE a.id=? AND a.deleted=0"
...
func (p *Dao) GetReviseFileByCond(c context.Context, node sqalx.Node, cond map[string]interface{}) (item *model.ReviseFile, err error) {
	...
	sqlSelect := fmt.Sprintf("SELECT a.id,a.file_name,a.file_url,a.seq,a.revise_id,a.deleted,a.created_at,a.updated_at FROM revise_files a WHERE a.deleted=0 %s", clause)
...
func (p *Dao) AddReviseFile(c context.Context, node sqalx.Node, item *model.ReviseFile) (err error) {
	sqlInsert := "INSERT INTO revise_files( id,file_name,file_url,seq,revise_id,deleted,created_at,updated_at) VALUES ( ?,?,?,?,?,?,?,?)"

	if _, err = node.ExecContext(c, sqlInsert, item.ID, item.FileName, item.FileURL, item.Seq, item.ReviseID, item.Deleted, item.CreatedAt, item.UpdatedAt); err != nil {
...
func (p *Dao) UpdateReviseFile(c context.Context, node sqalx.Node, item *model.ReviseFile) (err error) {
	sqlUpdate := "UPDATE revise_files SET file_name=?,file_url=?,seq=?,revise_id=?,updated_at=? WHERE id=?"

	_, err = node.ExecContext(c, sqlUpdate, item.FileName, item.FileURL, item.Seq, item.ReviseID, item.UpdatedAt, item.ID)
	if err != nil {
