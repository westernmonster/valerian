...
func (p *Dao) SetDiscussionCategoriesCache(c context.Context, topicID int64, m []*model.DiscussCategory) (err error) {
	key := def.DiscussionCategoriesKey(topicID)
	conn := p.mc.Get(c)
	defer conn.Close()
...
func (p *Dao) DiscussionCategoriesCache(c context.Context, topicID int64) (m []*model.DiscussCategory, err error) {
	key := def.DiscussionCategoriesKey(topicID)
	conn := p.mc.Get(c)
	defer conn.Close()
...
func (p *Dao) DelDiscussionCategoriesCache(c context.Context, topicID int64) (err error) {
	key := def.DiscussionCategoriesKey(topicID)
	conn := p.mc.Get(c)
	defer conn.Close()
