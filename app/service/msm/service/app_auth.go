...
func (s *Service) updateScope() (err error) {
	...
	if appAuthMap, err = s.dao.AllAppsAuth(c); err != nil {
		log.Errorf("The update scope process was abnormal, was blocked in DB!")
		return
	}
	if appTokenMap, err = s.dao.TreeAppInfo(c); err != nil {
		log.Errorf("The update scope process was abnormal, was blocked in service-tree service!")
		return
	}
	for sTreeID, sAuthMap := range appAuthMap {
		_, ok := appTokenMap[sTreeID]
		if !ok {
			log.Warnf("This app(%d) has no app_auth records in the service-tree service.", sTreeID)
			continue
		}
		...
		for appID, appAuth := range sAuthMap {
			...
			if !b {
				log.Warnf("This app(%d) has no app_auth records in the service-tree service.", appID)
				continue
			}
			scope := &model.Scope{
				AppTreeID:   appID,
...
func (s *Service) CheckSign(appID int64, sign string) bool {
	...
	if !ok || scope == nil {
		log.Errorf("CheckSign(%d) error(no msmTree info.)", appID)
		return false
	}
