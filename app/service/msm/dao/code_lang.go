...
func (p *Dao) CodesLang(c context.Context, node sqalx.Node) (codes map[int]map[string]string, lcode *model.CodeLangs, err error) {
	...
	sqlSelect := "SELECT a.id,a.locale,a.code,a.message,a.deleted,a.created_at,a.updated_at FROM code_langs a WHERE a.deleted=0 ORDER BY a.id"
