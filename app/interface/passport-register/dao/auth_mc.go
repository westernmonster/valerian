...
func (p *Dao) DelMobileValcodeCache(c context.Context, vtype int, mobile string) (err error) {
	key := vcMobileKey(vtype, mobile)
	conn := p.authMC.Get(c)
	defer conn.Close()
...
func (p *Dao) DelEmailValcodeCache(c context.Context, vtype int, mobile string) (err error) {
	key := vcEmailKey(vtype, mobile)
	conn := p.authMC.Get(c)
	defer conn.Close()
	if err = conn.Delete(key); err != nil {
