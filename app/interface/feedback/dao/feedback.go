...
func (p *Dao) GetFeedbacksByCond(c context.Context, node sqalx.Node, cond map[string]interface{}) (items []*model.Feedback, err error) {
	...
	sqlSelect := fmt.Sprintf("SELECT a.id,a.target_id,a.target_type,a.target_desc,a.target_user_id,a.feedback_type,a.feedback_desc,a.created_by,a.deleted,a.created_at,a.updated_at,a.verify_status,a.verify_desc FROM feedbacks a WHERE a.deleted=0 %s ORDER BY a.id DESC", clause)
...
func (p *Dao) GetFeedbackByID(c context.Context, node sqalx.Node, id int64) (item *model.Feedback, err error) {
	...
	sqlSelect := "SELECT a.id,a.target_id,a.target_type,a.target_desc,a.target_user_id,a.feedback_type,a.feedback_desc,a.created_by,a.deleted,a.created_at,a.updated_at,a.verify_status,a.verify_desc FROM feedbacks a WHERE a.id=? AND a.deleted=0"
...
func (p *Dao) GetFeedbackByCond(c context.Context, node sqalx.Node, cond map[string]interface{}) (item *model.Feedback, err error) {
	...
	sqlSelect := fmt.Sprintf("SELECT a.id,a.target_id,a.target_type,a.target_desc,a.target_user_id,a.feedback_type,a.feedback_desc,a.created_by,a.deleted,a.created_at,a.updated_at,a.verify_status,a.verify_desc FROM feedbacks a WHERE a.deleted=0 %s", clause)
...
func (p *Dao) AddFeedback(c context.Context, node sqalx.Node, item *model.Feedback) (err error) {
	sqlInsert := "INSERT INTO feedbacks( id,target_id,target_type,target_desc,target_user_id,feedback_type,feedback_desc,created_by,deleted,created_at,updated_at,verify_status,verify_desc) VALUES ( ?,?,?,?,?,?,?,?,?,?,?,?,?)"

	if _, err = node.ExecContext(c, sqlInsert, item.ID, item.TargetID, item.TargetType, item.TargetDesc, item.TargetUserID, item.FeedbackType, item.FeedbackDesc, item.CreatedBy, item.Deleted, item.CreatedAt, item.UpdatedAt, item.VerifyStatus, item.VerifyDesc); err != nil {
...
func (p *Dao) UpdateFeedback(c context.Context, node sqalx.Node, item *model.Feedback) (err error) {
	sqlUpdate := "UPDATE feedbacks SET target_id=?,target_type=?,target_desc=?,target_user_id=?,feedback_type=?,feedback_desc=?,created_by=?,updated_at=?,verify_status=?,verify_desc=? WHERE id=?"

	_, err = node.ExecContext(c, sqlUpdate, item.TargetID, item.TargetType, item.TargetDesc, item.TargetUserID, item.FeedbackType, item.FeedbackDesc, item.CreatedBy, item.UpdatedAt, item.VerifyStatus, item.VerifyDesc, item.ID)
	if err != nil {
