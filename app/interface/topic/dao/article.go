...
const (
	...
	_addArticleSQL    = "INSERT INTO articles( id,title,content,cover,locale,introduction,private,article_set_id,version_name,seq,created_by,deleted,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
	_updateArticleSQL = "UPDATE articles SET title=?,content=?,cover=?,locale=?,introduction=?,private=?,article_set_id=?,version_name=?,seq=?,created_by=?,updated_at=? WHERE id=?"
...
func (p *Dao) AddArticle(c context.Context, node sqalx.Node, item *model.Article) (err error) {
	if _, err = node.ExecContext(c, _addArticleSQL, item.ID, item.Title, item.Content, item.Cover, item.Locale, item.Introduction, item.Private, item.ArticleSetID, item.VersionName, item.Seq, item.CreatedBy, item.Deleted, item.CreatedAt, item.UpdatedAt); err != nil {
