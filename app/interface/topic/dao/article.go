...
const (
	...
	_addArticleSQL    = "INSERT INTO articles( id,title,cover,introduction,private,created_by,deleted,created_at,updated_at) VALUES ( ?,?,?,?,?,?,?,?,?)"
	_updateArticleSQL = "UPDATE articles SET title=?,cover=?,introduction=?,private=?,created_by=?,updated_at=? WHERE id=?"
...
func (p *Dao) AddArticle(c context.Context, node sqalx.Node, item *model.Article) (err error) {
	if _, err = node.ExecContext(c, _addArticleSQL, item.ID, item.Title, item.Cover, item.Introduction, item.Private, item.CreatedBy, item.Deleted, item.CreatedAt, item.UpdatedAt); err != nil {
...
func (p *Dao) UpdateArticle(c context.Context, node sqalx.Node, item *model.Article) (err error) {
	if _, err = node.ExecContext(c, _updateArticleSQL,
		item.Title, item.Cover, item.Introduction, item.Private, item.CreatedBy, item.UpdatedAt, item.ID,
	); err != nil {
