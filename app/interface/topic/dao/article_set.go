...
const (
	_addArticleVersionSQL       = "INSERT INTO article_versions( id,article_id,name,content,seq,deleted,created_at,updated_at) VALUES ( ?,?,?,?,?,?,?,?)"
	_updateArticleVersionSQL    = "UPDATE article_versions SET article_id=?,name=?,content=?,seq=?,updated_at=? WHERE id=?"
	_getArticleVersionSQL       = "SELECT a.* FROM  articles a  WHERE a.id=? AND a.deleted=0"
...
func (p *Dao) AddArticleVersion(c context.Context, node sqalx.Node, item *model.ArticleVersion) (err error) {
	if _, err = node.ExecContext(c, _addArticleVersionSQL,
		item.ID, item.ArticleID, item.Name, item.Content, item.Seq, item.Deleted, item.CreatedAt, item.UpdatedAt); err != nil {
...
func (p *Dao) UpdateArticleVersion(c context.Context, node sqalx.Node, item *model.ArticleVersion) (err error) {
	if _, err = node.ExecContext(c, _updateArticleVersionSQL, item.ArticleID, item.Name, item.Content, item.Seq, item.UpdatedAt, item.ID); err != nil {
		log.For(c).Error(fmt.Sprintf("dao.AddArticleVersion error(%+v), item(%+v)", err, item))
	}
