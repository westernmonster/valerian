...
func (p *Dao) AddDiscussionFile(c context.Context, node sqalx.Node, item *model.DiscussionFile) (err error) {
	sqlInsert := "INSERT INTO discussion_files( id,file_name,file_url,seq,discussion_id,deleted,created_at,updated_at) VALUES ( ?,?,?,?,?,?,?,?)"

	if _, err = node.ExecContext(c, sqlInsert, item.ID, item.FileName, item.FileURL, item.Seq, item.DiscussionID, item.Deleted, item.CreatedAt, item.UpdatedAt); err != nil {
...
func (p *Dao) UpdateDiscussionFile(c context.Context, node sqalx.Node, item *model.DiscussionFile) (err error) {
	sqlUpdate := "UPDATE discussion_files SET file_name=?,file_url=?,seq=?,discussion_id=?,updated_at=? WHERE id=?"

	_, err = node.ExecContext(c, sqlUpdate, item.FileName, item.FileURL, item.Seq, item.DiscussionID, item.UpdatedAt, item.ID)
	if err != nil {
...
func (p *Dao) DelDiscussionFiles(c context.Context, node sqalx.Node, discussionID int64) (err error) {
	sqlDelete := "UPDATE discussion_files SET deleted=1 WHERE discussion_id=? "

	if _, err = node.ExecContext(c, sqlDelete, discussionID); err != nil {
		log.For(c).Error(fmt.Sprintf("dao.DelDiscussionFiles err(%+v), discussion_id(%+v)", err, discussionID))
		return
	}
