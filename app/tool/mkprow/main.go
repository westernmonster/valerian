...
func generate() {
	filepath.Walk(".", func(path string, info os.FileInfo, err error) error {
		...
		if info.Name() == "OWNERS" && !info.IsDir() {
			...
			ts, ok := GlobalStatue.Template.JobConfig.Presubmits["platform/valerian"]
			if !ok {
			...
			for _, t := range ts {
				...
				GlobalStatue.Result.Presubmits["platform/valerian"] = append(GlobalStatue.Result.Presubmits["platform/valerian"], v)
			}
			return nil
		}
		return nil
	})
	//GlobalStatue.AlwaysRun.Presubmits["platform/valerian"][0].RunAfterSuccess = append(GlobalStatue.AlwaysRun.Presubmits["platform/valerian"][0].RunAfterSuccess, GlobalStatue.Result.Presubmits["platform/valerian"]...)
	//GlobalStatue.AlwaysRun.Presubmits["platform/valerian"][0].RunAfterSuccess = append(GlobalStatue.AlwaysRun.Presubmits["platform/valerian"][0].RunAfterSuccess, GlobalStatue.AppendTask.Presubmits["platform/valerian"]...)
	for _, v := range GlobalStatue.AppendTask.Presubmits["platform/valerian"] {
		v.Trigger = DefaultTriggerFor(v.Name)
		v.RerunCommand = DefaultRerunCommandFor(v.Name)
		GlobalStatue.Result.Presubmits["platform/valerian"] = append(GlobalStatue.Result.Presubmits["platform/valerian"], v)
	}
...
func replaceimage() {
	...
	at, ok := GlobalStatue.AppendTask.JobConfig.Presubmits["platform/valerian"]
	if !ok {
		fmt.Println("wrong project name")
		return
	}
	...
	ar, ok := GlobalStatue.AlwaysRun.JobConfig.Presubmits["platform/valerian"]
	if !ok {
		fmt.Println("wrong project name")
		return
	}
...
func generateLabel() (err error) {
	...
	GlobalStatue.TemplateLabels.Repos["platform/valerian"] = repo
