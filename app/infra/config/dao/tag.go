...
const (
	_getTagsByAppIDSQL = "SELECT a.* FROM tags a WHERE a.deleted=0 AND a.app_id=? ORDER BY a.id"
	_getTagByID        = "SELECT a.* FROM tags a WHERE a.id=? AND a.deleted=0"
	_getLastForce      = "SELECT a.id FROM tags a WHERE a.app_id=? AND a.build_id=? AND a.force=1 AND a.deleted=0"
)
...
func (p *Dao) UpdateTag(c context.Context, node sqalx.Node, item *model.Tag) (err error) {
	...
	if _, err = node.ExecContext(c, sqlUpdate, item.AppID, item.ConfigIds, item.Mark, item.Force, item.Operator, item.UpdatedAt, item.ID); err != nil {
