...
func (p *Dao) GetBuildsByAppIDs(c context.Context, node sqalx.Node, appIDs []int64) (items []*model.Build, err error) {
	items = make([]*model.Build, 0)
	strIDs := Int64Array2StringArray(appIDs)

	sqlSelect := "SELECT a.* FROM configs a WHERE a.deleted=0 AND a.app_id IN(?)"

	if err = node.SelectContext(c, &items, sqlSelect, strings.Join(strIDs, ",")); err != nil {
		log.For(c).Error(fmt.Sprintf("dao.GetBuildsByAppIDs err(%+v), app ids(%+v)", err, strIDs))
		return
	}
	return
}
