...
func file(c *mars.Context) {
	...
	if _, err = c.Writer.Write([]byte(data)); err != nil {
		log.Errorf("Response().Write(%v) error(%v)", data, err)
	}
}

// check check config version
func check(c *mars.Context) {
	var (
		err      error
		svr      string
		host     string
