...
func TestReplicate(t *testing.T) {
	Convey("test replicate", t, func() {
		...
		nodes := NewNodes(&dc.Config{HTTPClient: &mars.ClientConfig{Breaker: &breaker.Config{Window: xtime.Duration(time.Second),
			...
			Bucket:  10,
			Ratio:   0.5,
			Request: 100}, Timeout: xtime.Duration(time.Second), App: &mars.App{Key: "0c4b8fe3ff35a4b6", Secret: "b370880d1aca7d3a289b9b9a7f4d6812"}}, Mars: &dc.HTTPServers{Inner: &mars.ServerConfig{Address: "127.0.0.1:7171"}}, Nodes: []string{"api.bilibili.co", "uat-bilibili.co", "127.0.0.1:7171"}})
...
func TestReplicateSet(t *testing.T) {
	Convey("test replicate set", t, func() {
		nodes := NewNodes(&dc.Config{HTTPClient: &mars.ClientConfig{Breaker: &breaker.Config{Window: xtime.Duration(time.Second),
			...
			Bucket:  10,
			Ratio:   0.5,
			Request: 100}, Timeout: xtime.Duration(time.Second), App: &mars.App{Key: "0c4b8fe3ff35a4b6", Secret: "b370880d1aca7d3a289b9b9a7f4d6812"}}, Mars: &dc.HTTPServers{Inner: &mars.ServerConfig{Address: "127.0.0.1:7171"}}, Nodes: []string{"api.bilibili.co"}})
		nodes.nodes[0].client.SetTransport(gock.DefaultTransport)
