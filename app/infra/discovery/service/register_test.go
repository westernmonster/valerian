...
func newConfig() *dc.Config {
	return &dc.Config{HTTPClient: &mars.ClientConfig{Timeout: xtime.Duration(time.Second), Breaker: &breaker.Config{Window: xtime.Duration(time.Second),
		Sleep:   xtime.Duration(time.Millisecond * 100),
		Bucket:  10,
		Ratio:   0.5,
		Request: 100},
		App: &mars.App{Key: "0c4b8fe3ff35a4b6", Secret: "b370880d1aca7d3a289b9b9a7f4d6812"}},
		Mars: &dc.HTTPServers{Inner: &mars.ServerConfig{Address: "127.0.0.1:7171"}},
	}
}
