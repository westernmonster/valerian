...
func (p *TopicRepository) Insert(ctx context.Context, node sqalx.Node, item *Topic) (err error) {
	sqlInsert := "INSERT INTO topics( id,topic_set_id,name,cover,bg,introduction,is_private,allow_chat,allow_discuss,edit_permission,view_permission,join_permission,important,mute_notification,catalog_view_type,topic_type,topic_home,version_name,created_by,deleted,created_at,updated_at) VALUES ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"

	item.CreatedAt = time.Now().Unix()
	item.UpdatedAt = time.Now().Unix()

	_, err = node.ExecContext(ctx, sqlInsert, item.ID, item.TopicSetID, item.Name, item.Cover, item.Bg, item.Introduction, item.IsPrivate, item.AllowChat, item.AllowDiscuss, item.EditPermission, item.ViewPermission, item.JoinPermission, item.Important, item.MuteNotification, item.CatalogViewType, item.TopicType, item.TopicHome, item.VersionName, item.CreatedBy, item.Deleted, item.CreatedAt, item.UpdatedAt)
	if err != nil {
		err = tracerr.Wrap(err)
		return
	}
...
func (p *TopicRepository) Update(ctx context.Context, node sqalx.Node, item *Topic) (err error) {
	sqlUpdate := "UPDATE topics SET topic_set_id=?,name=?,cover=?,bg=?,introduction=?,is_private=?,allow_chat=?,allow_discuss=?,edit_permission=?,view_permission=?,join_permission=?,important=?,mute_notification=?,catalog_view_type=?,topic_type=?,topic_home=?,version_name=?,created_by=?,updated_at=? WHERE id=?"

	item.UpdatedAt = time.Now().Unix()

	_, err = node.ExecContext(ctx, sqlUpdate, item.TopicSetID, item.Name, item.Cover, item.Bg, item.Introduction, item.IsPrivate, item.AllowChat, item.AllowDiscuss, item.EditPermission, item.ViewPermission, item.JoinPermission, item.Important, item.MuteNotification, item.CatalogViewType, item.TopicType, item.TopicHome, item.VersionName, item.CreatedBy, item.UpdatedAt, item.ID)
	if err != nil {
		err = tracerr.Wrap(err)
		return
	}
